...
def menu_func(self, context):
	...
	row.operator(AngleSet.bl_idname, "angle")
	row.operator(LengthSet.bl_idname, "length")
	#分割
...
def register():
	...
	bpy.utils.register_class(AngleSet)
	...
	if AngleSet.bl_idname not in kc.keymap_items:
		kc.keymap_items.new(AngleSet.bl_idname, 'E', 'PRESS', shift = True, alt = True, ctrl = True)

def unregister():
	#与register对应，卸载功能
	bpy.utils.unregister_class(LengthSet)
	bpy.utils.unregister_class(AngleSet)
	...
	if AngleSet.bl_idname in kc.keymap_items:
		kc.keymap_items.remove(kc.keymap_items[AngleSet.bl_idname])
